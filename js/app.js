...
class HamsterApp {
    ...
    formatText(text, msgId = '', isMine = false) {
        if (!text) return '';
        ...
        text = text.replace(urlRegex, (match) => {
            ...
            let trailing = match.substring(url.length);
            return `<a href="${url}" target="_blank" style="color: ${linkColor}; text-decoration: underline; font-weight: 600;">${url}</a>${trailing}`;
        });
    ...
    listenForMessages(chatId) {
        ...
        this.messagesUnsubscribe = onSnapshot(q, async (snapshot) => {
            ...
            messagesHTML += renderedDocs.map(({ msgId, msg }, index) => {
                ...
                let contentStr = msg.isDeleted ? (this.lang === 'ar' ? '🚫 تم حذف هذه الرسالة' : '🚫 This message was deleted') : this.formatText(msg.text, msg.id || msgId, isMine);
                ...
                if (msg.isViewOnce) {
                    ...
                    contentStr = storyPreviewHTML + this.formatText(displayText, msg.id || msgId, isMine);
                    if (msg.edited) {
    ...
    async sendMessageWithMedia(chatId, imageData, isViewOnce = false) {
        ...
        try {
            ...
            await batch.commit();
            this.cancelReply();
            this.sendPushNotification(chat, displayLastMsg);
        } catch (e) {
    ...
    async sendPushNotification(chat, messageText) {
        if (!chat || chat.type === 'ai') return;
        const recipientIds = chat.memberIds.filter(id => id !== this.user.uid);
